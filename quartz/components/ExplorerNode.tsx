...
export class FileNode {
  ...
  displayName: string
  file: QuartzPluginData | undefined
  depth: number
  isClose: boolean
  ...
  constructor(slugSegment: string, displayName?: string, file?: QuartzPluginData, depth?: number) {
    ...
    this.depth = depth ?? 0
    this.isClose = file?.frontmatter?.isClose ?? false
  }
  ...
  private insert(fileData: DataWrapper) {
    ...
    if (fileData.path.length === 1) {
      if (nextSegment === "") {
        ...
        this.children.push(new FileNode("index", title, fileData.file, this.depth))
      } else {
        // direct child
    ...
    <>
      {node.file ? (
        ...
        <li>
          {node.name !== "" && (
            ...
            <div class="folder-container">
              ...
              <div 
                ...
                className={`folder ${node.depth === 1  ? 'folder-title-depth1' : ''}`}
              >
                {folderBehavior === "link" ? (
          ...
          <div class={`folder-outer ${node.depth === 0 || (isDefaultOpen && !node.isClose) ? "open" : ""}`}>
            <ul
              // Inline style for left folder paddings
              style={{
                paddingLeft: node.name !== "" ? "1.0rem" : "0",
              }}
              class="content"
