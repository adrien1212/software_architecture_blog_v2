...
- C'est donc l'interactor qui détient l'interface (on le voit avec la flèche)
- Le `Presenter` forme donc un pont entre l'interface requise par l'interactor et l'interface requise par la bibliothèque qu'on utilise pour faire l'IHM.
...
Le Controller fonctionne de la même manière, mais dans l'autre sens.
